...
export const PATHS = {
  root: ['root'] as MutablePath,
  simple: ['root', 'user'] as MutablePath,
  nested: ['root', 'user', 'profile', 'name'] as MutablePath,
  array: ['root', 'items', '0'] as MutablePath,
  map: ['root', 'map', 'map:0'] as MutablePath,
  set: ['root', 'set', 'set:0'] as MutablePath,
  deep: ['root', 'a', 'b', 'c', 'd', 'e'] as MutablePath,
};
...
export function createCollectionGraph() {
  return {
    ...
    nested: {
      array: [{ id: 1 }, { id: 2 }],
      map: new Map<string, any>([['inner', new Map([['deep', 'value']])]]),
    },
  };
}
 ...
 */
export function createDeepGraph(depth: number = 10): any {
 ...
 */
export function createLargeGraph(size: number = 100): any {
  const items: any[] = [];
 ...
 */
export function createMemoryImageMetadata(overrides: Partial<MemoryImageMetadata> = {}): MemoryImageMetadata {
  return {
 ...
 */
export function createMultipleMemoryImages(count: number = 3): MemoryImageMetadata[] {
  return Array.from({ length: count }, (_, i) =>
    createMemoryImageMetadata({
 ...
 */
export function createScriptEntry(overrides: Partial<ScriptHistoryEntry> = {}): ScriptHistoryEntry {
 ...
 */
export function createMultipleScripts(count: number = 10): ScriptHistoryEntry[] {
  return Array.from({ length: count }, (_, i) =>
    createScriptEntry({
      code: `// Script ${i + 1}\nthis.value${i} = ${i * 10};`,
 ...
 */
export function createLargeScript(sizeKB: number = 50): string {
